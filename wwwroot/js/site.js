...
const resetFrontId = () => {
    if (FrontIdImage) FrontIdImage.src = "";
    if (FrontIdImage) FrontIdImage.classList.add("d-none");

    if (FrontIdUploadArea) FrontIdUploadArea.classList.remove("d-none"); // show placeholder
    if (removeFrontIdBtn) removeFrontIdBtn.classList.add("d-none");

    if (FrontIdInput) FrontIdInput.value = "";
    if (removeFrontIdFlag) removeFrontIdFlag.value = "true"; // backend will delete it
};
...
document.addEventListener("DOMContentLoaded", () => {
    if (FrontIdImage && FrontIdImage.src && FrontIdImage.src.length > 5) {
        FrontIdImage.classList.remove("d-none");
        FrontIdUploadArea.classList.add("d-none");
        removeFrontIdBtn.classList.remove("d-none");
    }
...
const resetBackId = () => {
    if (BackIdImage) BackIdImage.src = "";
    if (BackIdImage) BackIdImage.classList.add("d-none");

    if (BackIdUploadArea) BackIdUploadArea.classList.remove("d-none"); // show placeholder
    if (removeBackIdBtn) removeBackIdBtn.classList.add("d-none");

    if (BackIdInput) BackIdInput.value = "";
    if (removeBackIdFlag) removeBackIdFlag.value = "true"; // backend will delete it
};
...
// if Model has an ID image already (optional integration)
document.addEventListener("DOMContentLoaded", () => {
    if (BackIdImage && BackIdImage.src && BackIdImage.src.length > 5) {
        BackIdImage.classList.remove("d-none");
        BackIdUploadArea.classList.add("d-none");
        removeBackIdBtn.classList.remove("d-none");
    }
...
const resetProfilePhoto = () => {
    ...
    if (profilePhotoPlaceholder) profilePhotoPlaceholder.innerHTML = '<i class="fas fa-user fa-3x text-muted"></i>';
    if (profilePhotoPlaceholder) profilePhotoPlaceholder.style.backgroundImage = "";
    if (profilePhotoPlaceholder) profilePhotoPlaceholder.classList.add("bg-light");
    if (profilePhotoPlaceholder) profilePhotoPlaceholder.classList.remove("has-photo");

    if (removeProfilePhotoBtn) removeProfilePhotoBtn.classList.add("d-none");

    if (profilePhotoInput) profilePhotoInput.value = "";
    if (removePhotoFlag) removePhotoFlag.value = "true"; // tell backend user removed photo
};
